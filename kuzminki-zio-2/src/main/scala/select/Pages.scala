...
class Pages[R](query: RenderedQuery[R], limit: Int) {
  ...
  def nextOpt: ZIO[Kuzminki, SQLException, Option[List[R]]] =
    ZIO.unless(isDone)(db.query(nextQuery)).map(markDone)

  def next: ZIO[Kuzminki, SQLException, List[R]] = nextOpt.map {
    case Some(rows) => rows
  ...
  def pageOpt(num: Int): ZIO[Kuzminki, SQLException, Option[List[R]]] = page(num).map {
    case Nil => None
    case rows => Some(rows)
  }

  def page(num: Int): ZIO[Kuzminki, SQLException, List[R]] = 
